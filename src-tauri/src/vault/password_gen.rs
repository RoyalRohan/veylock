...
/// BIP-39 standard wordlist (2048 words) providing 11 bits of entropy per word
pub const PASSPHRASE_WORDS: &[&str; 2048] = &[
...
pub fn generate_password_csprng(config: &PwGenConfig) -> Result<String, String> {
    if config.passphrase_mode {
        let count = config.word_count.clamp(3, 12) as usize;
    ...
    let filter_ambiguous = |slice: &[u8]| -> Vec<u8> {
        if config.exclude_ambiguous {
            slice.iter().copied().filter(|&b| !AMBIGUOUS.contains(&(b as char))).collect()
        } else {
            slice.to_vec()
        }
    };

    let mut required_bytes: Vec<u8> = Vec::new();
    let mut pool: Vec<u8> = Vec::new();

    if config.use_uppercase {
        let set = filter_ambiguous(UPPER);
        if !set.is_empty() {
            let ch = set[OsRng.gen_range(0..set.len())];
            required_bytes.push(ch);
            pool.extend_from_slice(&set);
        }
    }
    if config.use_lowercase {
        let set = filter_ambiguous(LOWER);
        if !set.is_empty() {
            let ch = set[OsRng.gen_range(0..set.len())];
            required_bytes.push(ch);
            pool.extend_from_slice(&set);
    ...
    if config.use_numbers {
        let set = filter_ambiguous(NUMBERS);
        if !set.is_empty() {
            let ch = set[OsRng.gen_range(0..set.len())];
            required_bytes.push(ch);
            pool.extend_from_slice(&set);
    ...
    if config.use_symbols {
        let set = filter_ambiguous(SYMBOLS);
        if !set.is_empty() {
            let ch = set[OsRng.gen_range(0..set.len())];
            required_bytes.push(ch);
            pool.extend_from_slice(&set);
    ...
    if pool.is_empty() {
        let lower = filter_ambiguous(LOWER);
        let numbers = filter_ambiguous(NUMBERS);
    ...
    let len = (config.length as usize).clamp(8, 128);
    let mut password_bytes = Vec::with_capacity(len);

    // If target length allows, include guaranteed characters from enabled sets
    if required_bytes.len() <= len {
        password_bytes.extend_from_slice(&required_bytes);
    }
    ...
    // Cryptographically secure shuffle using OsRng
